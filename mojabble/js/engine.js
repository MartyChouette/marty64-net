...
class Board {
  ...
  // tolerance (in screen px) enables forgiving touch hits: when no exact
  // hit lands, snap to the nearest FREE tile whose rect (expanded by the
  // tolerance) contains the point. Pass 0 for pixel-exact mouse behavior.
  getTileAtPos(sx, sy, tolerance = 0) {
    ...
    // Fuzzy fallback for touch: nearest free tile within the tolerance band.
    const tol = tolerance / this.scale;
    let best = null;
    let bestDist = Infinity;
    for (const tile of sorted) {
      if (!this.isFree(tile)) continue;
      const pos = tile.getScreenPos(this.offsetX, this.offsetY);
      if (bx >= pos.x - tol && bx <= pos.x + C.TILE_W + tol &&
          by >= pos.y - tol && by <= pos.y + C.TILE_H + tol) {
        const cx = pos.x + C.TILE_W / 2;
        const cy = pos.y + C.TILE_H / 2;
