...
class Game {
  ...
  _bindInput() {
    ...
    // tolerance > 0 gives touch a forgiving hit radius (see getTileAtPos).
    const handleTileTap = (sx, sy, tolerance = 0) => {
      this.audio._ensureContext();
      const tile = this.board.getTileAtPos(sx, sy, tolerance);
      if (!tile) {
        // A silent miss reads as "the game ignored me". Tick on touch misses.
        if (tolerance > 0) this.audio.playTick();
        return;
      }
    ...
    this.canvas.addEventListener('click', (e) => {
      if (this.state !== 'playing') return;
      // Ignore the synthetic click a browser fires after a real touch tap.
      if (this._lastTouchTime && performance.now() - this._lastTouchTime < 600) return;
      const pos = getMousePos(e);
      handleTileTap(pos.x, pos.y);
    });

    // Touch support - tap resolves on touchend so a finger can settle before
    // it commits, with a movement threshold so a drag/scroll is not a tap.
    let touchStart = null;
    let touchMoved = false;
    const TAP_MOVE_LIMIT = 16; // px of travel still counts as a tap
    const TOUCH_TOLERANCE = 18; // forgiving hit radius for fat fingers

    this.canvas.addEventListener('touchstart', (e) => {
      e.preventDefault();
      if (this.state !== 'playing') { touchStart = null; return; }
      const t = e.touches[0];
      const rect = this.canvas.getBoundingClientRect();
    ...
    this.canvas.addEventListener('touchmove', (e) => {
      ...
      if (Math.hypot(dx, dy) > TAP_MOVE_LIMIT) touchMoved = true;
    }, { passive: false });
    ...
    this.canvas.addEventListener('touchend', (e) => {
      ...
      if (touchStart && !touchMoved) {
        ...
        const rect = this.canvas.getBoundingClientRect();
        handleTileTap(t.clientX - rect.left, t.clientY - rect.top, TOUCH_TOLERANCE);
      }
      touchStart = null;
    }, { passive: false });
  }
  ...
  _submitToServer(payload) {
    fetch('scores.php?action=submit', {
      method: 'POST',
  ...
  _fetchGlobalScores() {
    return fetch('scores.php?action=scores')
      .then(r => r.ok ? r.json() : null)
      .then(data => {
        if (data) {
  ...
  _mpPollWaiting() {
    fetch(`room.php?action=poll&room=${this._mp.roomCode}&player=${this._mp.playerId}`)
      .then(r => r.ok ? r.json() : null)
      .then(data => {
        if (!data) return;
  ...
  _mpFinish() {
    ...
    .then(data => {
      if (data && data.finished) {
        ...
        return fetch(`room.php?action=poll&room=${this._mp.roomCode}&player=${this._mp.playerId}`)
          .then(r => r.ok ? r.json() : null);
      }
